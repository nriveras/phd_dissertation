...
% from img/ as written). The Makefile's `make sd` target overrides \GraphicsRoot
% on the command line (latexmk -usepretex) to point at a low-res copy of the
% images, so figures resolve from build/sd/img/ instead -- no per-file edits.
